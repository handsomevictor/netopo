...
关键设计决策：
1. **强制分两组**：`is_lan_ip()` 判断 10/172.16-31/192.168/127 为 LAN，其余为公网
2. **公网按 ISP 聚合**：用 hostname 关键词 + IP 前缀简单识别 Google/GitHub/Apple/Cloudflare/AWS/Akamai，其余归"其他"
3. **hostname 优先**：`node_display()` 输出 `"hostname (ip)"` 格式
4. **端口格式统一为 `PROTO:PORT x N`**（无括号，空格隔开）
5. **行宽 79 列**：`trunc()` 函数在超出时追加 `…`（与 `truncate()` 区分，后者用于固定宽度框）
6. **底部摘要**：遍历所有 edges 统计 Top3 端口，按 count 降序排列
